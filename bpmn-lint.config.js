...
function isAny$b( node, types ) {
	return types.some( ( type ) => is$k( node, type ) );
}

const index_esm = /* #__PURE__ */Object.freeze( {
	__proto__: null,
	is: is$k,
	isAny: isAny$b
} );
 ...
 */
const adHocSubProcess = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const conditionalFlows = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const endEventRequired = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const eventSubProcessTypedStartEvent = function () {
...
const global = function () {
	...
	function isReferenced( rootElement, referencingElements ) {
		if ( is$f( rootElement, 'bpmn:Error' ) ) {
			return referencingElements.some( ( referencingElement ) => is$f( referencingElement, 'bpmn:ErrorEventDefinition' ) &&
          rootElement.get( 'id' ) === referencingElement.get( 'errorRef' )?.get( 'id' ) );
		}

		if ( is$f( rootElement, 'bpmn:Escalation' ) ) {
			return referencingElements.some( ( referencingElement ) => is$f( referencingElement, 'bpmn:EscalationEventDefinition' ) &&
          rootElement.get( 'id' ) === referencingElement.get( 'escalationRef' )?.get( 'id' ) );
		}

		if ( is$f( rootElement, 'bpmn:Message' ) ) {
			return referencingElements.some( ( referencingElement ) => isAny$7( referencingElement, [
		...
		if ( is$f( rootElement, 'bpmn:Signal' ) ) {
			return referencingElements.some( ( referencingElement ) => is$f( referencingElement, 'bpmn:SignalEventDefinition' ) &&
...
function matchPattern( pattern ) {
	...
	return function ( el ) {
		...
		return every( pattern, ( val, key ) => el[ key ] === val );
...
function toMatcher( matcher ) {
	return isFunction( matcher ) ? matcher : ( e ) => e === matcher;
}
...
const noBpmndi = function () {
	...
	function check( node, reporter ) {
		...
		visualBpmnElements.forEach( ( element ) => {
			if ( !diBpmnReferences.includes( element.id ) ) {
...
function getAllDiBpmnReferences( definitionsNode ) {
	return flatten(
		definitionsNode.get( 'diagrams' ).map( ( diagram ) => {
			...
			return diElements.map( ( element ) => element.bpmnElement?.id );
		} )
	);
}
...
const noComplexGateway = checkDiscouragedNodeType$1( 'bpmn:ComplexGateway', 'no-complex-gateway' );
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const noDuplicateSequenceFlows = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const noGatewayJoinFork = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const noImplicitSplit = function () {
	...
	function check( node, reporter ) {
		...
		const outgoingWithoutCondition = outgoing.filter( ( flow ) => !hasCondition( flow ) && !isDefaultFlow( node, flow ) );
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const noImplicitStart = function () {
...
const noInclusiveGateway = checkDiscouragedNodeType( 'bpmn:InclusiveGateway', 'no-inclusive-gateway' );
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const noOverlappingElements = function () {
 ...
 * @return {Map<Object, Object>} map of di objects with element as key
 */
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const singleBlankStartEvent = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const singleEventDefinition = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const startEventRequired = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const subProcessBlankStartEvent = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const superfluousGateway = function () {
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
const superfluousTermination = function () {
...
const rules = {
	...
	"hallowelt/missing-gateway-label": "error",
	"hallowelt/missing-subprocess-label": "error",
	"hallowelt/non-unique-labels": "error",
	"hallowelt/non-unique-participant-labels": "error"
};
