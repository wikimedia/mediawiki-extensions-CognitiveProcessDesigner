...
function checkDiscouragedNodeType$2(type, ruleName) {
  ...
  return function() {
    ...
    return annotateRule$l(ruleName, {
      check
    });
 ...
 */
function annotateRule$l(ruleName, options) {
...
var adHocSubProcess = function() {
  ...
  function check(node, reporter) {
    ...
    flowElements.forEach(function(flowElement) {
      ...
      if (is$k(flowElement, 'bpmn:IntermediateCatchEvent')) {
  ...
  return annotateRule$k('ad-hoc-sub-process', {
    check
  });
...
var conditionalFlows = function() {
  ...
  return annotateRule$j('conditional-flows', {
    check
  });
...
var endEventRequired = function() {
  ...
  return annotateRule$i('end-event-required', {
    check
  });
};
...
var eventSubProcessTypedStartEvent = function() {
  ...
  return annotateRule$h('event-sub-process-typed-start-event', {
    check
  });
...
var fakeJoin = function() {
  ...
  return annotateRule$g('fake-join', {
    check
  });
...
var linkEvent = function() {
  ...
  function check(node, reporter) {
    ...
    if (!is$g(node, 'bpmn:FlowElementsContainer')) {
      return;
    }
...
function isLinkEvent(node) {
  ...
  if (!is$g(node, 'bpmn:Event')) {
    return false;
  }

  return eventDefinitions.some(
    definition => is$g(definition, 'bpmn:LinkEventDefinition')
  );
}

function getLinkName(linkEvent) {
  return linkEvent.get('eventDefinitions').find(def => is$g(def, 'bpmn:LinkEventDefinition')).name;
}

function isThrowEvent(node) {
  return is$g(node, 'bpmn:ThrowEvent');
}

function isCatchEvent(node) {
  return is$g(node, 'bpmn:CatchEvent');
}
...
var noDisconnected = function() {
  ...
  function check(node, reporter) {
    ...
    if (is$e(node.$parent, 'bpmn:AdHocSubProcess')) {
      return;
    }
...
var noImplicitSplit = function() {
  ...
  function check(node, reporter) {
    ...
    const outgoingWithoutCondition = outgoing.filter((flow) => {
      return !hasCondition$1(flow) && !isDefaultFlow(node, flow);
    });
...
var noImplicitEnd = function() {
  ...
  function check(node, reporter) {
    ...
    if (!isAny$5(node, [ 'bpmn:Event', 'bpmn:Activity', 'bpmn:Gateway' ])) {
      return;
    }
...
var noImplicitStart = function() {
  ...
  function isImplicitStart(node) {
    ...
    if (isAny$4(node, [ 'bpmn:StartEvent', 'bpmn:BoundaryEvent' ])) {
      return false;
    }
...
var startEventRequired = function() {
  ...
  function check(node, reporter) {
    ...
    if (!isAny$3(node, [
      'bpmn:Process',
      'bpmn:SubProcess'
    ]) || is$6(node, 'bpmn:AdHocSubProcess')) {
      return;
    }
...
var superfluousTermination = function() {
  ...
  function check(node, reporter) {
    ...
    if (!isAny$2(node, [ 'bpmn:Process', 'bpmn:SubProcess' ])) {
      return;
    }
...
var nonUniqueLabels = function () {
	...
	function check( node, reporter ) {
		...
		allElements = allElements.filter( ( element ) => isAny$1( element, subpageTypes ) );
...
var nonUniqueParticipantLabels = function () {
	...
	function check( node, reporter ) {
		if ( !is$1( node, 'bpmn:Collaboration' ) ) {
			return;
		}
...
const rules = {
  ...
  "mediawiki-cpd/label-required-except-gateways": "error"
};
...
cache['bpmnlint-plugin-mediawiki-cpd/label-required-except-gateways'] = labelRequiredExceptGateways;
